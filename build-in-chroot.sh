...
click chroot -a armhf -f ubuntu-sdk-15.04 -s vivid run CGO_ENABLED=1 GOARCH=arm GOARM=7 PKG_CONFIG_LIBDIR=/usr/lib/arm-linux-gnueabihf/pkgconfig:/usr/lib/pkgconfig:/usr/share/pkgconfig GOPATH=$DIR GOROOT=$GO_DIR CC=arm-linux-gnueabihf-gcc CXX=arm-linux-gnueabihf-g++ $GO_BIN_DIR/go install -ldflags '-extld=arm-linux-gnueabihf-gcc' -v -x password-shroud

# Copy files into click directory

cp $BIN_DIR/linux_arm/password-shroud $CLICK_DIR
cp -R $DIR/share $CLICK_DIR
cp $DIR/manifest.json $CLICK_DIR
cp $DIR/password-shroud.apparmor $CLICK_DIR
cp $DIR/password-shroud.desktop $CLICK_DIR
cp $DIR/password-shroud.png $CLICK_DIR
